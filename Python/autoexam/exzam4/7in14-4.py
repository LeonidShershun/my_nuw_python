'''Є чотирикутна схема польотів дронів з координатами (1, 2, 3, 4). У нас є словник points, ключі якого — кортежі, точки польоту
між координатами чотирикутника, вигляду (1, 2). Значення словника — це відстані між вказаними точками.
...
def calculate_distance(coordinates):
    coor = []
    dis=0
    for first, next in zip(coordinates, coordinates[1:]):
        s=[]
        if first > next:
            y=first
            x=next
